...
final class ProfileEditViewController: BaseViewController, UITableViewDataSource, UITableViewDelegate,
    ...
    // Firebase Storage 참조
    private let storage = Storage.storage()
  private var profileImageUrl = BehaviorSubject<String>(value: "")
    ...
    private func bindUI() {
        ...
        // 프로필 이미지 Observable 생성
      let profileImageObservable = profileImageUrl.asObservable()

        // ViewModel에 전달할 Input 바인딩 설정
        let input = ProfileEditViewModel.Input(
            ...
            profileImageUrl: profileImageObservable
        )

        // ViewModel을 사용한 데이터 바인딩
        bindViewModel(input: input)
    }

    // 사용자 정보 바인딩 메서드 추가
    private func bindViewModel(input: ProfileEditViewModel.Input) {
        let output = viewModel.transform(input: input)
        ...
        output.saveResult
            .drive(onNext: { [weak self] success in
                guard let self = self else { return }
                if success {
                    self.navigateToProfileController()  // 저장 후 ProfileController로 이동
                } else {
                    // 저장 실패 시 에러 처리
                    print("저장 실패")
                }
  ...
  private func uploadImageToFirebase(image: UIImage) {
      ...
      storage.reference().child(filePath).putData(imageData, metadata: metaData) { [weak self] metaData, error in
          ...
          self?.storage.reference().child(filePath).downloadURL { url, error in
              ...
              guard let downloadUrl = url else { return }
              // BehaviorSubject로 새로운 다운로드 URL을 전달
              self?.profileImageUrl.onNext(downloadUrl.absoluteString)
