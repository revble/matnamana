...
class ProfileEditViewModel {
    ...
    func transform(input: Input) -> Output {
        let saveResult = input.saveTap
            ...
            .flatMapLatest { [weak self] (name, nickname, shortDescription, userInfo) -> Observable<Bool> in
                ...
                // User 객체 생성
                let user = User(info: userInfoStruct, preset: [], friendList: [], userId: "some_user_id")
    ...
    private func saveUserData(user: User) -> Observable<Bool> {
        return Observable.create { observer in
            do {
                try self.db.collection("users").document(user.userId).setData(from: user) { error in
                    if let error = error {
                        print("Error writing document: \(error)")
                        observer.onNext(false)
                    } else {
